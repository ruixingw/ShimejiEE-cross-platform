...
/**
 * A move that can breed and/or scan for an affordance target, replacing
 * Move/BreedMove/ScanMove via {@code Characteristics="Breed,Scan"}.
 */
public class ComplexMove extends Move {
    ...
    public ComplexMove(java.util.ResourceBundle schema, final List<Animation> animations, final VariableMap context) {
    ...
    protected void tick() throws LostGroundException, VariableException {
        ...
        if (scanEnabled && target != null && target.get() != null) {
            boolean noMoveX = getMascot().getAnchor().x == target.get().getAnchor().x;
            boolean noMoveY = getMascot().getAnchor().y == target.get().getAnchor().y;
